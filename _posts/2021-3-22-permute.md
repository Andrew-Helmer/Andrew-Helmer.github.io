...
That subtitle is a little bit of a mouthful, but I've been wanting to talk about Andrew Kensler's <code>permute()</code> function for a while. <code>permute()</code> is used some in the graphics community, especially in offline rendering, but I think it's a really clever and beautiful function that could be useful for a lot of programmers. The techniques are also more widely known in cryptography\*, but the method is really useful far outside of that.
...
<code>uniform_rand_int(min, max)</code> is inclusive of both min and max, i.e. 
$$
\textnormal{swap\_idx} \in \{i, i+1, \dots, n-1\}
$$
